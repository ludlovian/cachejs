...
const
  ...
  { de, mand, bug } = require('./debug')('cachefile');


class CacheFile {

  constructor(realpath) {
    de&&mand.ok(realpath instanceof Path);
  ...
  wasRead(bytesRead) {
    de&&bug('%d bytes read', bytesRead);
    this.bytesRead += bytesRead;
    de&&bug('%d bytes read in total', this.bytesRead);

    const fileSize = this.realpath.meta.size;
    de&&bug('fileSize=%d', fileSize);

    if (this.bytesRead > config.preload.readPercentage * fileSize / 100) {
      de&&bug('more than threshold: %d vs %d', this.bytesRead, fileSize);
  ...
  requestPreload(v) {
    de&&bug('preload requested of %s', this.path);
