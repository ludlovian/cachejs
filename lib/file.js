 ...
 *
 * We set a timer for (about) a second to see if we are staying
 * on the same file.
 *
 * If so, and the queue is empty (we don't want to thrash it)
 * we schedule a load
 *
 */

function maybeCache(fileObj) {

    // are we on a differnt file?
    if (fileObj.path+'' !== lastFile.path+'') {
        if (lastFile.alarm)
